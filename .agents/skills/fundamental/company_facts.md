...
Special Parameters: - balance_sheet() also supports as_of parameter for point-in-time views
...
Use Specific Filters: More specific queries run faster
...
Limit Results Early: Use latest() or count() when appropriate
...
Chain Filters Logically: Put most selective filters first
...
if 'balance_sheet_metrics' in aapl_snapshot:
    ...
    for key, value in aapl_snapshot['balance_sheet_metrics'].items():
        print(f"{key}: {value}")
Performance Tips
Cache Company Objects: Reuse Company instances to leverage enhanced caching
Use as_dataframe=True: For bulk calculations, raw DataFrames are faster
Limit Periods: Request only the periods you need for analysis
Check Availability: Use if company.facts: before accessing financial data
Choose Format Wisely: Use concise_format=True for display, False for calculations
Cache LLM Context: Store to_llm_context() results for repeated AI analysis
Batch Web Rendering: Use iter_with_values() to skip empty items
