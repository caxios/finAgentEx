...
def get_price_analysis(ticker: str):
    ...
    hist, _ = fetch_market_data(ticker)
...
2.  **Scrape**: If you find a high-impact headline (e.g., earnings release, bad news, major product launch), use `scrape_website` on its URL to get the full details. Don't scrape everything, only the most critical 1-2 items.
...
def duckduckgo_search_tool(query: str):
    ...
    # Fetch 20 results to enable time-based filtering
    raw_results = DDGS().news(query, region='us-en', max_results=20)
